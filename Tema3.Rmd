...
nuevos_datos<-ggplot(df_filtrado, aes(x = date, y = new_deaths_smoothed_per_million, color = location, group = location)) + geom_line() + scale_y_continuous(breaks = seq(0, 18, by = 2)) + scale_x_date(date_labels = "%b %d %Y", breaks = seq(min(df_filtrado$date), max(df_filtrado$date), length.out = 5), limits = c(min(df_filtrado$date), as.Date("2023-03-07")))+scale_color_manual(values=colores)+geom_line(data = filter(df_filtrado, location == "Spain"), size = 0.9, col  =  "grey")+labs(title = "Daily new confirmed COVID-19 deaths per million people", x = "date", y = "new_deaths_smoothed_per_million", color = "paises")
nuevos_datos
